...
void ModelTime(TString filename){

 
  Double_t Eg,Mmiss,M1,M2,fgID,Sig,Mmiss_M1,time;
  ...
  TF1* ftimes=new TF1("times","gaus(0)",-10,10);
  ftimes->SetParameters(1,1,1);
  ...
  for(Int_t i=0;i<Nev;i++){
    ...
    if(gRandom->Uniform()>0.5){
      ...
      M1=fM1s->GetRandom();
      M2=fM2s->GetRandom();
      time=ftimes->GetRandom();
      Mmiss=fMmisss->GetRandom();
      Sig=1;
      tsig->Fill();
     }
    else if(gRandom->Uniform()>0.5){
      Eg=gRandom->Uniform(3,4);
      M1=fM1b->GetRandom();
      M2=fM2b->GetRandom();
      time=ftimes->GetRandom();
    ...
    else {//random time coincidences
      Eg=gRandom->Uniform(3,4);
      time=gRandom->Uniform(-10,10);
      M1=gRandom->Uniform(0,10);
      M2=gRandom->Uniform(0,10);
      Mmiss=gRandom->Uniform(0,10);
      Sig=-2;
     tbg->Fill();
