...
void THSSkeleton::CreateRooFitEventsPDF(TString pdfName,TString obsNames,TString parNames){
  ...
  for(Int_t io=0;io<obss->GetEntries();io++)
   ...
   if(!is_cat[io]) ContinueLineAfter(TString("  Double_t mc")+obss->At(io)->GetName()+Form("=fvecReal[fTreeEntry*fNvars+%d];",nv++));
    else ContinueLineAfter(TString("  Int_t mc")+obss->At(io)->GetName()+Form("=fvecCat[fTreeEntry*fNcats+%d];",nc++));
