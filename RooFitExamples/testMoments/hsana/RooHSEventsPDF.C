...
void RooHSEventsPDF::initGenerator(Int_t code)
{
  ...
  if(fParent->GetMaxValue()==0||fParent->CheckChange()){	
    ...
    if(code==1){	
      ...
      for(Int_t i=1;i<fNTreeEntries;i++){
	fTreeEntry=i;
	value=evaluateMC();
...
Double_t RooHSEventsPDF::analyticalIntegral(Int_t code,const char* rangeName) const
{
  ...
  if(ihigh>fNTreeEntries) ihigh=fNTreeEntries;
   if(code==1){
     cout<<"INTEGRAL "<<fNTreeEntries<<" "<<fvecReal.size()<<" "<<fvecReal[0]<<endl;
...
Bool_t RooHSEventsPDF::CheckRange(const char* rangeName) const{
  ...
  for(UInt_t i=0;i<fProxSet.size();i++){
    ...
    if(!var->inRange(fMCVar[i],TString(rangeName).Data())) return kFALSE;
  }
  return kTRUE;
...
Bool_t RooHSEventsPDF::SetEvTree(TTree* tree,TString cut,Long64_t ngen){
  ...
  Long64_t NValid=elist->GetN();
  for(Long64_t iEvent=0;iEvent<NValid;iEvent++){
    ...
    for(UInt_t ip=0;ip<ProxSize;ip++)
      fvecReal[iEvent*ProxSize+ip]=fMCVar[ip];
    for(UInt_t ip=0;ip<CatSize;ip++)
      fvecCat[iEvent*CatSize+ip]=fMCCat[ip];
  }
...
Bool_t RooHSEventsPDF::AddProtoData(RooDataSet* data){
  ...
  while(RooAbsArg* arg=(RooAbsArg*)iter()){
    ...
    // if(TString("fuID")==TString(arg->GetName())) continue; //already exists
    // 
  ...
  for(Long64_t id=0;id<fNTreeEntries;id++){
 	...
 	fvecReal[id*fNvars+protoDataForVar[ip]]=dataVars->getRealValue(fProxSet[protoDataForVar[ip]]->GetName());
     }
     for(UInt_t ip=0;ip<protoDataForCat.size();ip++){
       fvecCat[id*fNcats+protoDataForCat[ip]]=dataVars->getCatIndex(fCatSet[protoDataForCat[ip]]->GetName());
     }
  //      cout<<"vecs "<<id<<" "<<fvecReal[id*fNvars+0]<<" "<<fvecReal[id*fNvars+1]<<" "<<fvecReal[id*fNvars+2]<<" "<<fvecReal[id*fNvars+3]<<" "<<fvecReal[id*fNvars+4]<<" "<<endl;
  // cout<<"cats "<<fvecCat[id*fNcats+0]<<" "<<fvecCat[id*fNcats+1]<<endl;
