...
void THipo::InitFile(TString filename){
  ...
  //fReader=nullptr;
  if(!fReader)fReader=new hipo::reader();
...
Bool_t THipo::NextEvent(){//Read next event in file
  ...
  for(UInt_t ig=0;ig<fBanks.size();ig++){
    ...
    fCurBank->ResetEntry();
    if(fOutTree)fCurBank->ReadEvent();//read into vectors
...
Bool_t THipoItem::TreeBranch(TString branchname0,TTree* outtree){
  ...
  if(!fIsVector)outtree->Branch(fNBranchname,&fNEntries,fNBranchname+"/I");   
  if(fIsVector)
...
Bool_t THipoBankParser::ConfigBank3(THipoBank* bank){
  ...
  for(uint v = 0; v < code.size(); v++){
    std::string vcode=code[v].c_str();
      ...
      case 1: bank->AddItem(new THipoItemB(fReader->getBranch<int8_t>(bank->GetName(),CodeToItem(vcode).Data()),bank->GetEntryAddress()),CodeToItem(vcode).Data()); break;
      case 2: bank->AddItem(new THipoItemS(fReader->getBranch<int16_t>(bank->GetName(),CodeToItem(vcode).Data()),bank->GetEntryAddress()),CodeToItem(vcode).Data()); break;
      case 3: bank->AddItem(new THipoItemI(fReader->getBranch<int32_t>(bank->GetName(),CodeToItem(vcode).Data()),bank->GetEntryAddress()),CodeToItem(vcode).Data()); break;
      case 4: bank->AddItem(new THipoItemF(fReader->getBranch<float>(bank->GetName(),CodeToItem(vcode).Data()),bank->GetEntryAddress()),CodeToItem(vcode).Data()); break;
      case 5: bank->AddItem(new THipoItemD(fReader->getBranch<double>(bank->GetName(),CodeToItem(vcode).Data()),bank->GetEntryAddress()),CodeToItem(vcode).Data()); break;
      case 8: bank->AddItem(new THipoItemL(fReader->getBranch<int64_t>(bank->GetName(),CodeToItem(vcode).Data()),bank->GetEntryAddress()),CodeToItem(vcode).Data()); break;
      default: break;
    } 
  }
...
Bool_t THipoBankParser::ReConfigBank3(THipoBank* bank){
  if(!bank->IsConfigured()) cout<<"Warning THipoBankParser::ReConfigBank3 bankk not yet configured"<<" "<<bank->GetName()<<endl;
  //Use the code string to parse the reader::getBranch
  fDictionary=fReader->getSchemaDictionary();
  std::vector<std::string>  code = fDictionary->getSchema(bank->GetName()).branchesCode();
  //bank->SetGroup(fDictionary->getSchema(bank->GetName()).getGroup());
  ...
  for(uint v = 0; v < code.size(); v++){
    std::string vcode=code[v].c_str();
       switch(CodeToType(vcode)){
      case 1: dynamic_cast<THipoItemB*>(bank->GetItem(CodeToItem(vcode)))->ResetNode(fReader->getBranch<int8_t>(bank->GetName(),CodeToItem(vcode).Data())); break;
      case 2: dynamic_cast<THipoItemS*>(bank->GetItem(CodeToItem(vcode)))->ResetNode(fReader->getBranch<int16_t>(bank->GetName(),CodeToItem(vcode).Data())); break;
...
int THipoBankParser::CodeToType(string code){
  ...
  if(type_str==string("int8_t")) return 1;
  if(type_str==string("int16_t")) return 2;
  if(type_str==string("int32_t")) return 3;
  if(type_str==string("float")) return 4;
  if(type_str==string("double")) return 5;
  if(type_str==string("int64_t")) return 8;
  return 0;
}
