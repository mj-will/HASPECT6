...
extern int putBOS(BOSbank *bcs, int lun, char *list);
...
extern void *getNextBank(BOSbank *bcs, bankHeader_t *bank);

/*get a specific bank*/
extern void *getGroup(BOSbank *, const char *name, int j);

/*make a new bank*/
extern void *makeBank(BOSbank *bcs, char *bankname, int banknum, int ncol, int nrow);

/*Drop a bank*/
extern void *dropBank(BOSbank *bcs, char *bankname, int banknum);
...
/*format a new bank*/
extern void formatBank(char *bankname, char *bankformat);

/*wrapper for call to blist_ */
extern void bankList(BOSbank *bcs, char *list, char *banklist);

/* open a fpack format file for reading*/
extern void open_fpack(char *filename);

/* get a BOS type event returns 1 if success returns 0 if fail (eof)*/
extern int GetData();

extern int fileLength(char *datafile);
